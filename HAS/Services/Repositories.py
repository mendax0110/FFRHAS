...
class SensorDataRepository:
    def __init__(self, user:str, pwd:str):
        self.__client = pymongo.MongoClient(f"mongodb://{user}:{pwd}@mongodb:27017")  # connection string
    ...
    def get_from_sensor_for_range(self, sensor_name:datetime,start_date:datetime, stop_date:datetime):
        start_date = start_date.astimezone(timezone.utc)
        stop_date = stop_date.astimezone(timezone.utc)
        data = self.__test.find({"sensor_name": "temperature_sensor_1",
                                 "timestamp": { "$gte": start_date.isoformat(), "$lte": start_date.isoformat() }
                                 })
        return dumps(data)

    def write_one(self, data:str):
